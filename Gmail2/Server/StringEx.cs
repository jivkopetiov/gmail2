...
namespace Gmail2.Server
{
    public static class StringEx
    {
        ...
        {
            return builder.AppendFormat(CultureInfo.InvariantCulture, format, parameters);
        }
            ...
            string replacedText = smartFormatRegexPattern.Replace(pattern, match =>
            {
                ...
                else
                {
                    if (!cache.TryGetValue(key, out value))
                    {
                        ...
                        value = Convert.ToString(prop.GetValue(template, null), CultureInfo.InvariantCulture);
                        cache.Add(key, value);
                    }
                }
